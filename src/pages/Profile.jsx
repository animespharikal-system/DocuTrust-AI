...
export default function Profile() {
  ...
  const [email, setEmail] = useState('alex.carter@docutrust.ai');
  const [password, setPassword] = useState('••••••••');
  
  // API Key States
  const [apiKeys, setApiKeys] = useState([
    { id: 1, name: 'Production Webhook Link', key: 'dt_live_2a8b9f71ee01fc938df12', created: '2026-05-12' },
    { id: 2, name: 'Testing Local Daemon', key: 'dt_test_88f9dacb2748aa01e2390f', created: '2026-06-01' }
  ]);
  ...
  const handleGenerateKey = (e) => {
    ...
    const prefix = newKeyName.toLowerCase().includes('prod') ? 'dt_live_' : 'dt_test_';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="border-b border-zinc-900 pb-5">
        <h1 className="text-xl font-bold tracking-tight text-white sm:text-2xl">Workspace Settings</h1>
        <p className="text-zinc-400 text-xs mt-1">Manage your developer credentials, API tokens, security logs, and billing tiers.</p>
      </div>
        ...
        <div className="md:col-span-3 space-y-6">
          ...
          {activeTab === 'account' && (
            <div className="rounded-xl border border-zinc-800 bg-zinc-900/10 p-5 space-y-5">
              <h3 className="font-bold text-white text-xs uppercase tracking-wider border-b border-zinc-900 pb-3">User Profile info</h3>
              ...
              <div className="space-y-1.5">
                <label className="text-[9px] font-bold text-zinc-500 uppercase tracking-widest">Workspace Password</label>
              ...
              <div className="pt-4 border-t border-zinc-900">
                <button
                  onClick={() => alert('Account configuration changes saved (simulation)')}
          ...
          {activeTab === 'developer' && (
            <div className="rounded-xl border border-zinc-800 bg-zinc-900/10 p-5 space-y-5">
              <h3 className="font-bold text-white text-xs uppercase tracking-wider border-b border-zinc-900 pb-3">Developer Credentials</h3>
              <p className="text-xs text-zinc-400 leading-relaxed">
                Generate authorization tokens to integrate the DocuTrust document integrity daemon with your local dev pipelines (CI/CD, API webhooks).
              </p>
              ...
              <div className="space-y-3 mt-4">
                <h4 className="text-[9px] font-bold text-zinc-500 uppercase tracking-widest">Active Credentials Registry</h4>
                
                {apiKeys.length === 0 ? (
                  <div className="text-center py-6 text-xs text-zinc-500 font-mono">No active API keys found.</div>
                ) : (
                  <div className="space-y-2.5">
                    {apiKeys.map((key) => {
                      const displayedStr = key.key.substring(0, 12) + '...' + key.key.substring(key.key.length - 8);
                      return (
                        <div key={key.id} className="rounded-lg border border-zinc-850 bg-zinc-950 p-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
          ...
          {activeTab === 'security' && (
            <div className="rounded-xl border border-zinc-800 bg-zinc-900/10 p-5 space-y-5">
              <h3 className="font-bold text-white text-xs uppercase tracking-wider border-b border-zinc-900 pb-3">Security Policies</h3>
              ...
              <div className="flex items-center justify-between p-4 rounded-xl bg-zinc-950 border border-zinc-900">
                <div className="space-y-0.5 pr-6">
                  ...
                  <p className="text-[10px] text-zinc-500 leading-normal">Protect logins and cryptographic operations by requiring OTP tokens.</p>
                </div>
                <button
              ...
              <div className="space-y-3.5">
                <h4 className="text-[9px] font-bold text-zinc-500 uppercase tracking-widest">Active login sessions</h4>
                <div className="rounded-lg bg-zinc-950 border border-zinc-900 p-4 space-y-3.5 text-xs">
                  <div className="flex justify-between items-start border-b border-zinc-900 pb-3">
                    <div>
                      <div className="font-bold text-zinc-200">Chrome (Windows 11) - Current session</div>
                      <div className="text-[10px] text-zinc-500 font-mono mt-0.5">IP: 198.162.2.4 • Dallas, US</div>
                    </div>
                  ...
                  <div className="flex justify-between items-start">
                    <div>
                      <div className="font-bold text-zinc-350">DocuTrust CLI Daemon v2.4</div>
                      <div className="text-[10px] text-zinc-550 font-mono mt-0.5">IP: 54.81.90.12 • AWS N. Virginia, US</div>
                    </div>
                    <button
          ...
          {activeTab === 'billing' && (
            <div className="rounded-xl border border-zinc-800 bg-zinc-900/10 p-5 space-y-5">
              <h3 className="font-bold text-white text-xs uppercase tracking-wider border-b border-zinc-900 pb-3">Billing System & Tier</h3>
              ...
              <div className="rounded-xl bg-gradient-to-br from-indigo-950/20 to-zinc-900/40 border border-indigo-500/20 p-5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div className="space-y-1">
                  <div className="text-[10px] font-bold text-indigo-300 uppercase tracking-widest">Active plan</div>
                  <div className="text-base font-bold text-white mt-1">DocuTrust Professional Tier</div>
                  <p className="text-[10px] text-zinc-400 leading-normal">Next renewal date: July 24, 2026 ($49.00/month billing).</p>
                </div>
                <button
              ...
              {/* Payment Card */}
              <div className="space-y-3.5">
                <h4 className="text-[9px] font-bold text-zinc-500 uppercase tracking-widest">Payment Instruments</h4>
                <div className="rounded-lg bg-zinc-950 border border-zinc-900 p-4 flex items-center justify-between text-xs">
                  <div className="flex items-center gap-3">
                    <div className="h-7 w-11 bg-zinc-900 rounded border border-zinc-850 flex items-center justify-center font-bold text-[9px] italic text-zinc-500">VISA</div>
                    <div>
                      <div className="font-bold text-zinc-200">Visa ending in 4821</div>
                      <div className="text-[10px] text-zinc-500 mt-0.5">Expires 12 / 2029</div>
                    </div>
                  </div>
                  <button
